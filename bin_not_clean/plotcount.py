...
def plot_word_counts(counts, limit=10):
    """
    Given a list of (word, count, percentage) tuples, plot the counts as a histogram. Only the first limit tuples are plotted.
    """
...
def typeset_labels(labels=None, gap=5):
    """
    Given a list of labels, create a new list of labels such that each label is right-padded by spaces so that every label has the same width, then is further right padded by ' ' * gap.
    """
...
def get_ascii_bars(values, truncate=True, maxlen=10, symbol="#"):
    """
    Given a list of values, create a list of strings of symbols, where each strings contains N symbols where N = ()(value / minimum) (maximum - minimum)) * (maxlen / len(symbol)).
    """
    maximum = max(values)
    if truncate: 
...
def plot_ascii_bars(values, labels=None, screenwidth=80, gap=2, truncate=True):
    """
    Given a list of values and labels, create right-padded labels for each label and strings of symbols representing the associated values.
    """
    if not labels:
        try:
    ...
    bars = get_ascii_bars( values, maxlen=screenwidth - gap - len(labels[0]), truncate=truncate)
