...
def load_text( filename):
    """
    Load lines from a plain-text file and return these as a list, with trailing newlines stripped.
    """
...
def save_word_counts( filename, counts ):
    """
    Save a list of [word, count, percentage] lists to a file, in the form  "word count percentage", one tuple per line.
    """
...
def update_word_counts(line, counts ):
  """ 
  This function updates a word count dictionary, but it's kinda messy. 
  It removes delimiters with replace, splits on spaces, iterates... 
  (Trust me, there are better ways...)
  """

  # Lots of string replacements, not very efficient
  for purge in DELIMITERS: 
    line = line.replace(purge, " " ) 
  
  # No error handling, what if line is empty? 
  words = line.split()
  
  for word in words:
    # Lowercase and strip in a separate line, not very readable
    word = word.lower().strip()

    # Typo in increment, should be counts[word] += 1
    if word in counts:
      counts[word] = count[word] + 1  # This should be counts[word] += 1
    else:
      counts[word] = 1
...
def calculate_word_counts( lines ):
    ...
    word counts (mapping words to counts of their frequencies). DELIMITERS are removed before the string is parsed. The function is case-insensitive and words in the dictionary are in lower-case.
    """ 
    counts = { }
    for line in lines:
...
def word_count_dict_to_tuples(counts, decrease=True):
    ...
    return sorted( list(counts.items()), key=lambda key_value: key_value[1], reverse=decrease )
...
def word_count(input_file, output_file, min_length=1):
    """
    Load a file, calculate the frequencies of each word in the file and save in a new file the words, counts and percentages of the total  in
